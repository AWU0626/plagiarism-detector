...
public class PlagiarismDetectorTest {
	...
	@Test
	public void test() {
		String directory = "plagiarism-detector/docs";
		...
		try {
			...
			e.printStackTrace();
			fail("ERROR! Could not read corpus of documents: be sure that /" + directory
					+ " is a subdirectory of where you started Java, or is a subdirectory of the root of your IDE's project");
		} catch (Exception e) { // oops, got an exception
			e.printStackTrace();
			fail("INCORRECT OUTPUT: detectPlagiarism throws " + e.toString()
		...
		for (Map.Entry<String, Integer> entry : entries) {
			...
			if (count == 0) { // bwa242.txt-sra42.txt: 10
				...
				if ((key.contains("bwa242.txt") && key.contains("bwa249.txt"))
			...
			} else if (count == 3 || count == 4 || count == 5) { // edo20.txt-edo26.txt: 7 ; bwa132.txt-bwa137.txt: 7 ;
																	// bwa132.txt-bwa133.txt: 7
				if (value != 7) {
				...
				if (key.contains("edo20.txt") && key.contains("edo26.txt")) {
					...
					fail("INCORRECT OUTPUT: incorrect key/value pair in Map; key [" + key
